...
boot_alloc(uint32_t n)
{
	...
	//cprintf("Next memory at %x\n", ROUNDUP(nextfree+n, PGSIZE));
	if(!n) {
		return nextfree;
	}   
	/*size_t roundn = ROUNDUP(n, PGSIZE);
	if(roundn / PGSIZE > npages){
	    panic("Out of memory!");
	}*/
	result = nextfree;
...
mem_init(void)
{
	...
	// Your code goes here:
	boot_map_region(kern_pgdir, UPAGES, ROUNDUP((sizeof(struct PageInfo)
			* npages), PGSIZE), PADDR(pages), (PTE_U | PTE_P));
	//////////////////////////////////////////////////////////////////////
	...
	// LAB 3: Your code here.
	boot_map_region(kern_pgdir, UENVS, ROUNDUP((sizeof(struct Env) * NENV),
...
page_init(void)
{
	...
	for(i = 1; i < npages; i++){
		...
		pa <= ((uint32_t)boot_alloc(0) - KERNBASE) >> PGSHIFT)) 
		&& (pages[i].pp_ref == 0)) {
			cprintf("page error:i %d|n", i);
		}
	}
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    for(; begin < end; begin += PGSIZE) {
        ...
        if(!pte || begin >= ULIM || (*pte & perm) != perm) {
            user_mem_check_addr = begin < (uint32_t) va ? (uintptr_t)
                                                        va : begin;
            return -E_FAULT;
        }
    }
